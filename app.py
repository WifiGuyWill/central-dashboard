...
#for c in base_resp["clients"]:
#    resp = (f'ip: {c.get("ip_address", "")}, mac: {c.get("macaddr", "")}')
...
def central_info():
    base_resp = central.command(apiMethod=apiMethod,
                            apiPath=apiPath,
                            apiParams=apiParams)
    return render_template("index.html", base_resp = base_resp)
...
def client_info():
    base_resp = central.command(apiMethod=apiMethod,
                            apiPath=apiPath,
                            apiParams=apiParams)
    return render_template("clients.html", base_resp = base_resp)
...
def ap_info():
    base_resp = central.command(apiMethod=apiMethod,
                            ...
                            apiParams=apiParams)
    return render_template("aps.html", base_resp = base_resp)

@app.route("/test")
def test():
    aps = central.command(apiMethod=apiMethod,
                            apiPath="/monitoring/v2/aps",
                            apiParams=apiParams)
    
    clients = central.command(apiMethod=apiMethod,
                            apiPath=apiPath,
                            apiParams=apiParams)
    return render_template("test.html", aps = aps, clients = clients)
'''
@app.route("/")
def index():
...
def device_info():
    ...
    switches = central.command(apiMethod="GET",
                            apiPath="/monitoring/v2/switches")
